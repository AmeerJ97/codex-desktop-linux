# Contributors

Thanks to everyone who has contributed to codex-desktop-linux!

This fork preserves credit to the original [ilysenko/codex-desktop-linux](https://github.com/ilysenko/codex-desktop-linux) project, its contributors, and its MIT license. Downstream canonical copies should keep this attribution visible.

## Core Contributors

- [@ilysenko](https://github.com/ilysenko)
- [@Leay15](https://github.com/Leay15)

## Contributors

- [@prichardsondev](https://github.com/prichardsondev)
- [@monkey-w1n5t0n](https://github.com/monkey-w1n5t0n)
- [@GuimelGMC](https://github.com/GuimelGMC) — Fedora RPM updater support
- [@quantmind-br](https://github.com/quantmind-br) — Arch Linux (pacman) package support
- [@moabualruz](https://github.com/moabualruz) — user-local desktop integration
- [@fndov](https://github.com/fndov) — NixOS support
- [@khushveer007](https://github.com/khushveer007) — auto-update mechanism
- [@DhanushSantosh](https://github.com/DhanushSantosh) — desktop entry lifecycle hardening
- [@LAP87](https://github.com/LAP87) — Arch Linux updater support
- [@lalalandau](https://github.com/lalalandau) — installer hardening and fresh-rebuild support
