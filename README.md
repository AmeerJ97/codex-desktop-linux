...
Native-package-only launcher behavior such as desktop-entry hints, `codex-update-manager` session bootstrapping, and the fallback launch-time update check lives in `packaging/linux/codex-packaged-runtime.sh`, which the generated launcher loads only when present inside a packaged install.
